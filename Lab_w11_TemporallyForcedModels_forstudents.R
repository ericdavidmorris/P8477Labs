## Lab 9: Temporally forced models
...
## SEIR model with sinusoidal seasonal forcing
...
N=1; E0=I0=N*1e-3; S0=N*6e-2; 
R0=17; alpha=1/8; gamma=1/5; mu=1/50; omega=2*pi/365; 
state=c(S=S0, E=E0,I=I0);

parms0=c(beta=R0*gamma,alpha=alpha,gamma=gamma,mu=mu); # parameters for the SEIRdem model
parms1=c(beta0=R0*gamma,beta1=.02,omega=omega,alpha=alpha,gamma=gamma,mu=mu); # for the SEIRsine model
times=seq(1,1000*365,by=1) # need to run 1000 yrs to have the 3rd sim stable!


## NOTE IT WILL TAKE A MINUTE OR SO TO RUN EACH SIMULATION - SO WAIT, WAIT, WAIT...
...
# plot the last 10 years here on your own:

par(mfrow = c(1,1), cex=1, mar=c(3,3,1,1), mgp=c(1.8, 0.5,0))
plot(tsim1[,'time'], tsim1[,'I'], ylab="%I", xlab="Time (Days)", 
     ylim=c(0.072, 0.073), lty=1, col=c('red'), main = 'SEIR model with and without seasonal forcing')
lines(tsim0[,'time'], tsim0[,'I'], lty=2, col=c('blue'))
legend('topright', c('SEIR with sinusoidal seasonal forcing', 'SEIR without seasonal forcing'), 
       lty=c(1,2), col=c('red', 'blue'),cex = 0.9, bty = 'n')



plot(tsim0[, 'time'],tsim0[, 'I'],type='l', lwd=2, col=c('red'),
     lty=1, main = 'SEIR model without seasonal forcing', cex.main = 1, ylab='Incidence',xlab='Time')

plot(tsim1[, 'time'],tsim1[, 'I'],type='l', lwd=2, col=c('red'),
     lty=1, main = 'SEIR model with sinusoidal seasonal forcing', cex.main = 1, ylab='Incidence',xlab='Time)')
...
#bt= beta0/correction.factor * (1 + b.term * Term[50])
#bt
