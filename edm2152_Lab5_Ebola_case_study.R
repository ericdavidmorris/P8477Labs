...
plot(sim[,'cumInci'], ylab = 'Cumulative incidence', xlab = 'Days', type = 'l', lwd = 2)
...
axis(1,at = df.bar,lab = 1:20)
lines(x = df.bar, y = inci, lwd = 2, col = "red")
lines(x = df.bar, y = inci2,lwd = 2, col = "blue") 
legend('topleft',c('Observed','Simulated: with intervention', 'Simulated: no intervention'),
       lty = c(NA,1,1), pch = c(0,NA,NA), col = c('black','blue', 'red'), cex = .8, bty = 'n')
...
for (i in 1:length(zh)) {
  ...
  z.H = zh[i]; # intervention in the hospital
  z.F = 0.875; # intervention in safe burial
  parmsCtrl = c(alpha = alpha, # incubation period: 7 days
                gamma.h = gamma.h, # from onset to hopspitalization: 5 days
                gamma.d = gamma.d, # from onset to death: 9.6 days
...
axis(1,at = df.bar,lab = 1:20)
matlines(x = df.bar, y = res,lty = 1, lwd = 2,col = rainbow(ncol(res))) # use 'x=df.bar' to match with weeks
legend(0,ymax/1.2,c(paste('zh=',zh,sep = '')),
       lty = 1, lwd = 2,col = rainbow(ncol(res)),bty = 'n', cex = .8)
...
for (i in 1:length(zf)) {
  z = 0.88; # intervention in the community
  z.H = 0.75; # intervention in the hospital
  z.F = zf[i]; # intervention in safe burial
  parmsCtrl = c(alpha = alpha, # incubation period: 7 days
                gamma.h = gamma.h, # from onset to hopspitalization: 5 days
                gamma.d = gamma.d, # from onset to death: 9.6 days
                gamma.i = gamma.i, # from onset to end of infectiousness for survivors: 10 days
...
axis(1,at = df.bar,lab = 1:20)
matlines(x = df.bar, y = res,lty = 1, lwd = 2,col = rainbow(ncol(res))) # use 'x=df.bar' to match with weeks
legend(0,ymax/1.2,c(paste('zf=',zf,sep = '')),
       lty = 1, lwd = 2,col = rainbow(ncol(res)),bty = 'n', cex = .8)
...
time=seq(6,12,by=2);
res=matrix(0,20,length(time))
for (i in 1:length(time)){
  times1=seq(1,time[i]*7,by=1);
  times2=seq(time[i]*7, 20*7, by=1);
  z=0.88; # intervention in the community
  z.H=0.75; # intervention in the hospital
  z.F=0.875; # intervention in safe burial
  parmsCtrl=c(alpha=alpha, # incubation period: 7 days
...
axis(1,at=df.bar,lab=1:20)
matlines(x=df.bar, y=res,lty=1, lwd=2,col=rainbow(ncol(res))) # use 'x=df.bar' to match with weeks
legend(0,ymax/1.2,c(paste('Week = ',time,sep='')),
       lty=1, lwd=2,col=rainbow(ncol(res)),bty='n',cex=.8)
